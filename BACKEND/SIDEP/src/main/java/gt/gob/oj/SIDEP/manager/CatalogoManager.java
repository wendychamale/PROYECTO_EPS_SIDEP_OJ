...
public class CatalogoManager {
  ...
  public List<Map<String, Object>> catalogoDependenciasArea(String P_PROCESO_ESTADO_AREA) throws Exception {
    List<Map<String, Object>> salida = new ArrayList<>();
    ConnectionsPool c = new ConnectionsPool();
    Connection conn = c.conectar();
    CallableStatement call = conn.prepareCall("call " + "RRHH" + ".PKG_CATALOGOS.PROC_GET_DEPENDENCIAS_AREA(?,?,?)");
